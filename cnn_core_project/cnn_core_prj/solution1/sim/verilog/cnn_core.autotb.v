...
`define AUTOTB_DUT      cnn_core
`define AUTOTB_DUT_INST AESL_inst_cnn_core
`define AUTOTB_TOP      apatb_cnn_core_top
`define AUTOTB_LAT_RESULT_FILE "cnn_core.result.lat.rb"
`define AUTOTB_PER_RESULT_TRANS_FILE "cnn_core.performance.result.transaction.xml"
`define AUTOTB_TOP_INST AESL_inst_apatb_cnn_core_top
...
wire [31:0] input_layer_transaction;
wire axi_s_input_layer_TVALID;
wire axi_s_input_layer_TREADY;

AESL_axi_s_input_layer AESL_AXI_S_input_layer(
    .clk(AESL_clock),
    .reset(AESL_reset),
    .TRAN_input_layer_TDATA(input_layer_TDATA),
    .TRAN_input_layer_TVALID(axi_s_input_layer_TVALID),
    .TRAN_input_layer_TREADY(axi_s_input_layer_TREADY),
    .ready(input_layer_ready),
    .done(input_layer_done),
    .transaction(input_layer_transaction));

assign input_layer_ready = input_layer_ready_reg | ready_initial;
assign input_layer_done = 0;
...
begin
    ...
    initial begin : proc_gen_axis_internal_ready_input_layer
        input_layer_ready_reg = 0;
        @ (posedge ready_initial);
        forever begin
            @ (ap_c_n_tvin_trans_num_input_layer or input_layer_transaction);
            if (ap_c_n_tvin_trans_num_input_layer > input_layer_transaction) begin
                input_layer_ready_reg = 1;
            end else begin
                input_layer_ready_reg = 0;
            end
        end
    ...
    `define STREAM_SIZE_IN_input_layer "../tv/stream_size/stream_size_in_input_layer.dat"
    
    initial begin : gen_ap_c_n_tvin_trans_num_input_layer
        integer fp_input_layer;
        reg [127:0] token_input_layer;
        integer ret;
        
        ap_c_n_tvin_trans_num_input_layer = 0;
        end_input_layer = 0;
        wait (AESL_reset === 1);
        
        fp_input_layer = $fopen(`STREAM_SIZE_IN_input_layer, "r");
        if(fp_input_layer == 0) begin
            $display("Failed to open file \"%s\"!", `STREAM_SIZE_IN_input_layer);
            $finish;
        end
        read_token(fp_input_layer, token_input_layer); // should be [[[runtime]]]
        if (token_input_layer != "[[[runtime]]]") begin
            $display("ERROR: token_input_layer != \"[[[runtime]]]\"");
            $finish;
        end
        size_input_layer = 0;
        size_input_layer_backup = 0;
        while (size_input_layer == 0 && end_input_layer == 0) begin
            ap_c_n_tvin_trans_num_input_layer = ap_c_n_tvin_trans_num_input_layer + 1;
            read_token(fp_input_layer, token_input_layer); // should be [[transaction]] or [[[/runtime]]]
            if (token_input_layer == "[[transaction]]") begin
                read_token(fp_input_layer, token_input_layer); // should be transaction number
                read_token(fp_input_layer, token_input_layer); // should be size for hls::stream
                ret = $sscanf(token_input_layer, "%d", size_input_layer);
                if (size_input_layer > 0) begin
                    size_input_layer_backup = size_input_layer;
                end
                read_token(fp_input_layer, token_input_layer); // should be [[/transaction]]
            end else if (token_input_layer == "[[[/runtime]]]") begin
                $fclose(fp_input_layer);
                end_input_layer = 1;
            end else begin
                $display("ERROR: unknown token_input_layer");
                $finish;
            end
        end
        forever begin
            @ (posedge AESL_clock);
            if (end_input_layer == 0) begin
                if ((input_layer_TREADY & input_layer_TVALID) == 1) begin
                    if (size_input_layer > 0) begin
                        size_input_layer = size_input_layer - 1;
                        while (size_input_layer == 0 && end_input_layer == 0) begin
                            ap_c_n_tvin_trans_num_input_layer = ap_c_n_tvin_trans_num_input_layer + 1;
                            read_token(fp_input_layer, token_input_layer); // should be [[transaction]] or [[[/runtime]]]
                            if (token_input_layer == "[[transaction]]") begin
                                ...
                                size_input_layer = size_input_layer_backup;
                                $fclose(fp_input_layer);
                                end_input_layer = 1;
                            end else begin
                                $display("ERROR: unknown token_input_layer");
