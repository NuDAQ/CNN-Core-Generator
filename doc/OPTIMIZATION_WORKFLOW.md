...
### HGQ Keras vs hls4ml Dense Weight Offset

There is a recorded systematic score offset between original HGQ Keras and
hls4ml/RTL output:
...
The large component comes from dense weight quantization mismatch. hls4ml uses
uniform `ap_fixed<9,5>` for dense weights, while the HGQ model uses
heterogeneous per-weight quantization.

Documented fix:

```bash
python3 scripts/extract_hgq_weights.py
```

This writes HGQ-quantized weights into the firmware weight files. It is a
weight-file correction, not a data-path optimization.
