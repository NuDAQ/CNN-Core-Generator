...
void pack_aria_input(scalar_T *logical_input, hls::stream<waveform_x2_t> &packed_input) {
    for (unsigned word_index = 0; word_index < 128; word_index++) {
        waveform_x2_t word;
        for (unsigned row = 0; row < 2; row++) {
            for (unsigned channel = 0; channel < 4; channel++) {
                word[row * 4 + channel] = logical_input[(word_index * 2 + row) * 4 + channel];
            }
        }
...
void cnn_core_float(float *waveform, float *layer9_out) {
    hls::stream<waveform_x2_t> waveform_ap("waveform");
